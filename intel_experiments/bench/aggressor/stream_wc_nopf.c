/*
 * stream_wc_nopf.c — WC (MOVNTDQA) streaming aggressor with hardware
 *                    prefetchers DISABLED (Condition E)
 *
 * Identical to stream_wc.c (MOVNTDQA non-temporal loads on a WB-mapped region)
 * except it writes MSR 0x1A4 = 0xF before running to disable all four hardware
 * prefetchers, and restores MSR on exit.
 *
 * This is the "software-only streaming, no hardware concurrency" datapoint for
 * the Mitigation Trap Plan Phase 1(b): it shows that without the hardware
 * prefetcher supplying in-flight depth, per-core streaming bandwidth stays low
 * even with non-temporal loads — i.e. NT/PREFETCHNTA software prefetching cannot
 * recover the bandwidth that WB+prefetch achieves.
 *
 * Usage:
 *   ./stream_wc_nopf --cpu 1 --node 0 --region-gb 1 --duration-sec 60
 */
...
/* Sequential MOVNTDQA sweep; 4 × 16B NT loads per cache line. */
...
int main(int argc, char **argv)
{
    ...
    if (((uintptr_t)buf & 15) != 0) {
        fprintf(stderr, "stream_wc_nopf: ERROR: region not 16-byte aligned\n");
        exit(1);
    }
    memset(buf, 0xAB, region_size);

    fprintf(stderr, "stream_wc_nopf (MOVNTDQA, prefetch OFF): cpu=%d node=%d "
    ...
    printf("{\"cpu\": %d, \"condition\": \"E_wc_nopf\", \"region_gb\": %zu, "
